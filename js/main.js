...
function init_threeScene(spec) {
  ...
  // CREATE THE MASK
  /*const maskLoader = new THREE.GLTFLoader(loadingManager);
 
  maskLoader.load("./models/face/faceLowPolyEyesEarsFill2.glb" , (faceMask) => {
    capMesh = faceMask.scene;
    const mat = new THREE.MeshPhongMaterial({
      map:new THREE.TextureLoader().load('models/face/face.png'),
     //  color:0xffffff,
     // transparent:true,
    //  opacity:1,
  ...
  //  capMesh.computeVertexNormals();
 faceMesh = new THREE.Mesh(capMesh, mat);
 ...
 const maskLoader = new THREE.BufferGeometryLoader(loadingManager);
  maskLoader.load('./models/face/faceLowPolyEyesEarsFill2.json', function (maskBufferGeometry) {
    ...
    void main() {\n\
      ...
      float borderCoeff = smoothstep(0.0, .55, vNormalDotZ);\n\
